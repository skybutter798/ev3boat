        ...
        Swal.fire({
            ...
            html: `
                ...
                <div class="pagination" style="margin-top:15px">
                    <button ${data.prev_page_url ? '' : 'disabled'} data-page="${data.current_page - 1}">Previous</button>
                    <button ${data.next_page_url ? '' : 'disabled'} data-page="${data.current_page + 1}">Next</button>
                </div>
            `,
            width: '40%',
        });
                ...
                <tbody>
                    ${items.map(item => `
                        <tr>
                            ...
                            <td><button data-item-id="${item.id}" class="purchase-button">Purchase</button></td>
                        </tr>
                    `).join('')}
                </tbody>
            </table>
    ...
    stoneSound.play();
      Swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          .then(response => {
            const data = response.data;
            const luckyNumber = data.LuckyNumber || data.NotLuckyNumber; 
        ...
        if (response.data.status === 'success') {
            ...
            if (itemId == 1) {
                ...
                if (remainingClicksDiv) {
                    remainingClicksDiv.textContent = `You have ${updatedClicks} clicks left for today.`;
        ...
        Swal.fire({
            ...
            html: `
                ...
                <div class="pagination-controls" style="margin-top:15px">
                    <button id="prevPage" ${currentPage <= 1 ? 'disabled' : ''} data-page="${currentPage - 1}">Previous</button>
                    <span>Page ${currentPage} of ${lastPage}</span>
                    <button id="nextPage" ${currentPage >= lastPage ? 'disabled' : ''} data-page="${currentPage + 1}">Next</button>
                </div>
            `,
            showCloseButton: true,
            showConfirmButton: false,
...
document.getElementById('code').addEventListener('click', function() {
    Swal.fire({
        ...
        showLoaderOnConfirm: true,
        preConfirm: (code) => {
            return axios.post('/redeem-code', { code: code })
                .then(response => {
                    const data = response.data;
                    if (data.status === 'success') {
                        Swal.fire({
